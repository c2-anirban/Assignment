        ...
        try {
            bcrypt.compare(password, user.password, async (err, result) => {
                if (result) {
                    ...
                    return res.status(200).send({ error: false,user: user, message: "Logged in successfully!", token: token });
                } else {
                    return res.status(422).send({ error: true, message: 'Invalid Credentials' });
                }
            })
...
exports.signup = async function (req, res) {
    ...
    if (!req.body.name || !req.body.email || !req.body.phone || !req.body.password || !req.body.confirm_password || !req.body.gender) {
